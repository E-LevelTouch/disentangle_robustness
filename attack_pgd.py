...
def pkg_pgd(model, X, y, epsilon=8 / 255, num_steps=20,
            step_size=0.01, random_start=True):
    out = model(X)
    is_correct_natural = (out.max(1)[1] == y).float().cpu().numpy()
    X_pgd = LinfPGDAttack(model, loss_fn=nn.CrossEntropyLoss, eps=epsilon,
                           nb_iter=num_steps, eps_iter=step_size, rand_init=random_start)

    adv_correct = np.reshape(
            (model(X_pgd).max(1)[1] == y).float().cpu().numpy(),
            [-1, 1])
    return is_correct_natural, np.array([adv_correct])
